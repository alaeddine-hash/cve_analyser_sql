...
vendor_name_present = 0  # Compteur pour 'vendor_name' non null (Yes)
vendor_name_absent = 0  # Compteur pour 'vendor_name' null (No)
...
for cve in data:
    ...
    if isinstance(os_names, list) and isinstance(os_versions, list):
        ...
        if os_version in ['all', 'all versions', 'All versions', 'null'] or os_version == '':
            is_all_versions = True
        elif os_version:
            is_exact_versions = True
    ...
    # Vérifier si 'vendor_name' est présent ou non
    vendor_name = cve.get('vendor_name')
    if vendor_name not in [None, '', [], {}, 'null']:
        vendor_name_present += 1
    else:
        vendor_name_absent += 1
...
if total_cves > 0:
    ...
    vendor_name_present_percentage = (vendor_name_present / total_cves) * 100
    vendor_name_absent_percentage = (vendor_name_absent / total_cves) * 100
else:
    ...
    vendor_name_present_percentage = vendor_name_absent_percentage = 0
...
print(f"CVEs avec 'vendor_name' présent: {vendor_name_present} ({vendor_name_present_percentage:.2f}%)")
print(f"CVEs avec 'vendor_name' absent: {vendor_name_absent} ({vendor_name_absent_percentage:.2f}%)")
